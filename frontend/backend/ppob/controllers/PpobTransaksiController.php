...
class PpobTransaksiController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $date = $cookies->getValue('TGL');
        $ACCESS_GROUP = $cookies->getValue('ACCESS_GROUP');
        $STORE_ID = $cookies->getValue('STORE_ID');
        // $date=Yii::$app->response->cookies->remove('TGL');
        // $ACCESS_GROUP=Yii::$app->response->cookies->remove('ACCESS_GROUP');
        // $STORE_ID=Yii::$app->response->cookies->remove('STORE_ID');

        // print_r($date.$ACCESS_GROUP.$STORE_ID);die();
		if ($date!='' && $ACCESS_GROUP!='' && $STORE_ID!=''){
            $tanggal = explode(" - ", $date);
			$cari=['tgllama'=>$tanggal[0],'tglbaru'=>$tanggal[1],'ACCESS_GROUP'=>$ACCESS_GROUP,'STORE_ID'=>$STORE_ID];			
            // print_r($cari);die();
        }else{
    ...
    public function actionPencarianIndex(){
        ...
        $modelPeriode = new PpobTransaksi();
		if ($modelPeriode->load(Yii::$app->request->post())) {
            // print_r();die();
            Yii::$app->response->cookies->add(new \yii\web\Cookie([
                'name'=>'TGL',
                'value'=>''.Yii::$app->request->post('TGL').'',
            ]));
            Yii::$app->response->cookies->add(new \yii\web\Cookie([
                'name'=>'ACCESS_GROUP',
...
public function actionSubstore() {
    ...
    if (!empty($paramCari)) {
           $data= PpobTransaksi::find()->select('STORE_ID,ACCESS_GROUP')->where(['ACCESS_GROUP'=>$paramCari])->count();
           if ($data>0) {
            $access= PpobTransaksi::find()->select('STORE_ID,ACCESS_GROUP')->where(['ACCESS_GROUP'=>$paramCari])->groupBy(['STORE_ID'])->all();
